...
| **`stride`** *(de extracción — B, no el de inferencia ni el de las convoluciones)* | **5** | **cerrado por un lado** *(25/25)* | **1** *(no aplicado)* | Monótono y sin rupturas: 16 → 0,8901 · 8 → 0,9159 · 4 → 0,9277 · 2 → 0,9328 · **1 → 0,9383**. 1 contra 16: `p` = **0,0079**. ⚠ **Gana el extremo**: dentro de δ = 0,0026 no queda ningún otro brazo, así que no hay saturación en el rango y el eje **no cierra por arriba** — con ventana 16 no se puede mirar más denso que 1 px. No es eje de `space`: es campo de **B**, un dataset por valor | [#15](2026/08-agosto/2026-08-27-stride-estudio.md) |
